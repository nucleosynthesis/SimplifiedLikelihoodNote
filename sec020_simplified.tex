...
The multivariate gaussian approximation may be expected to perform less well 
for values of systematic uncertainty pulled significantly from their central
values. To test this the two systematic uncertainties are set to their $+1.5\sigma$
values when generating the data (the covariance is unchanged). Figure~\ref{fig:likelihoodscan_bias} shows the resultant 
value of $q(\mu)$ as a function of $\mu$. The agreement between the full and simplfied likelihoods is significantly worsened 
compared to Figure~\ref{fig:likelihoodscan} and highlights the limitations of the gaussian approximation
when systematic uncertainties are pulled significantly.

\begin{figure}[hbt]
  \begin{center} 
   \includegraphics[width=1.5\cmsFigWidth]{figures/r_bias.pdf}
   \caption{The value of $q(\mu)$, when including a large systematic bias in generating the data, defined using the simplified likelihood using the full covariance matrix (open blue points), and defined using the full likelihood (solid black line).}
   \label{fig:likelihoodscan_bias} 
  \end{center}
\end{figure}

Finally, to illustrate an extreme case where the simplified likelihood approach may not be appropriate,
a systematic uncertainty with a highly asymmetric probability density function is 
included on the normalisation of the background (a log normal with
mean=0 and standard deviation = 0.5, corresponding to a skewness of 1.8). Figure ~\ref{fig:likelihoodscan_asymm} shows the resultant
value of $q(\mu)$ as a function of $\mu$ for the full and simplified likelihoods. 
It is clear that the there is a bias in the estimate of $\hat{\mu}$ from the simplified likelihood. 
This is due to the use of symmetric, gaussian constraints to model a highly asymmetric uncertainty.

\begin{figure}[hbt]
  \begin{center} 
   \includegraphics[width=1.5\cmsFigWidth]{figures/r_asymm.pdf}
   \caption{The value of $q(\mu)$, when including a highly asymmetric uncertainty on the background normalisation, defined using the simplified likelihood using the full covariance matrix (open blue points), and defined using the full likelihood (solid black line).}
