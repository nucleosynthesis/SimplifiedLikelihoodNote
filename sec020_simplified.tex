...
The probability density function for the nuisance parameters is modelled as a multivariate Gaussian with the mean vector identified with $\tilde{\boldsymbol{\theta}}=0$. The 
likelihood of Equation.~\ref{eq:generic-likelihood} can then be expressed as,

\begin{equation}
\mathcal{L}(\mu, \boldsymbol{\theta}) =  \prod_{i=1}^{N} \dfrac{(\mu \cdot s_{i}+b_{i}+\theta_{i})^{n_{i}} e^{-(\mu \cdot s_{i}+b_{i}+\theta_{i})} }{n_{i}!} \cdot \mathrm{exp} 
\left(-\dfrac{1}{2} \boldsymbol{\theta}^{T}\mathrm{\mathbf{V}}^{-1}\boldsymbol{\theta} \right)
